...
def process_allSignalGroups(result_section, all_signal_groups):
    ...
    for signalgroup in all_signal_groups:
        ...
        signal_readables = [
            signal.get("signalReadable") for signal in signalgroup.get("signals")
        ]
        ...
        if (
            "allMitreTechniques" in signalgroup
            and len(signalgroup.get("allMitreTechniques", [])) > 0
        ):
            for mitre in signalgroup.get("allMitreTechniques", []):
                signalgroup_mitre_rs = ResultSection(
                    signalgroup_description,
                    body_format=BODY_FORMAT.TEXT,
                    body=", \n".join(signal_readables),
                )
                signalgroup_mitre_rs.set_heuristic(
                    SIGNALS_HEURISTIC_ID[signal_verdict.lower()],
                    signature=mitre.get("name", None),
                    attack_id=mitre.get("ID", None),
                )
                result_section.add_subsection(signalgroup_mitre_rs)
        else:
            signalgroup_rs = ResultSection(
                signalgroup_description,
                body_format=BODY_FORMAT.TEXT,
                body=", \n".join(signal_readables),
            )
            ...
            for signal in signalgroup.get("signals", []):
                ...
                if "Matched YARA rule" in signalReadable:
                    result_section.add_tag(
                        "file.rule.yara", signal.get("additionalInfo")
                    )
...
def process_iocs(result_section, iocs):
    origin_type_map = {
        "INPUT_FILE": "static",
        "INPUT_CONTEXT": "static",
        "EXTRACTED_FILE": "static",
        "DOWNLOADED_FILE": "static",
        "URL_REDIRECT": "static",
        "CONTENT_PARSE": "static",
        "EMAIL_BODY": "static",
        "EXTERNAL_PARSER": "static",
        "JAVA_DECOMPILATION": "static",
        "DOTNET_DECOMPILATION": "static",
        "APK_DECODING": "static",
        "AUTOIT_DECOMPILATION": "static",
        "IMAGE_OCR": "static",
        "IMAGE_QR_CODE": "static",
        "OLE_PARSE": "static",
        "PCODE_PARSE": "static",
        "DOMAIN_RESOLVE": "static",
        "LNK_PARSE": "static",
        "XOR_DECODING": "static",
        "INTERNAL": "static",
        ...
        "WINDOWSSCRIPTFILE_EMULATION": "dynamic",
        "CSVFILE_EMULATION": "dynamic",
        "REGISTRYFILE_EMULATION": "dynamic",
        "XSLFILE_EMULATION": "dynamic",
        "CHMFILE_EMULATION": "dynamic",
        "MSHTA_EMULATION": "dynamic",
        "VBA_EMULATION": "dynamic",
        "BATCH_SCRIPT_EMULATION": "dynamic",
        "POWERSHELL_EMULATION": "dynamic",
        "COMMANDLINE_EMULATION": "dynamic",
        "PORTEX_EMULATION": "dynamic",
    }

    for ioc_key, ioc_values in iocs.items():
        for ioc_value in ioc_values:
            data = ioc_value.get("data")
            analysis_type = []
            for origin in ioc_value.get("origins"):
                origin_type = origin.get("type")
                if origin_type_map.get(origin_type) not in analysis_type:
                    analysis_type.append(origin_type_map.get(origin_type))

            if ioc_key == "email":
                result_section.add_tag("network.email.address", data)

            if ioc_key == "ip":
                for analysis in analysis_type:
                    result_section.add_tag(f"network.{analysis}.ip", data)
            if ioc_key == "domain":
                for analysis in analysis_type:
                    result_section.add_tag(f"network.{analysis}.domain", data)
            if ioc_key == "url":
                for analysis in analysis_type:
                    result_section.add_tag(f"network.{analysis}.uri", data)
    ...
    for resource_key, resource in resources.items():
        ...
        if "metaData" in resource:
            ...
            if "pdf:docinfo:modified" in metadata:
                result_section.add_tag(
                    "file.pdf.date.modified", metadata["pdf:docinfo:modified"]
                )
            ...
            if "dc:creator" in metadata:
                print("CREATOR!")
                result_section.add_tag(
                    "file.ole.summary.author", metadata["dc:creator"]
                )
            if "Company" in metadata:
                result_section.add_tag("file.ole.summary.company", metadata["Company"])
            if "dcterms:created" in metadata:
                result_section.add_tag(
                    "file.ole.summary.create_time", metadata["dcterms:created"]
                )
            if "meta:print-date" in metadata:
                result_section.add_tag(
                    "file.ole.summary.last_printed", metadata["meta:print-date"]
                )
            if "meta:last-author" in metadata:
                result_section.add_tag(
                    "file.ole.summary.last_saved_by", metadata["meta:last-author"]
                )
            if "dc:subject" in metadata:
                result_section.add_tag(
                    "file.ole.summary.subject", metadata["dc:subject"]
                )
